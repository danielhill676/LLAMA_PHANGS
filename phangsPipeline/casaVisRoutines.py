...
def find_spws_for_line(
    ...
    """
    import inspect
    print("DEBUG: USING casaVisRoutines.find_spws_for_line FROM:", inspect.getfile(inspect.currentframe()))
    ...
    for this_spw in vm.spwInfo.keys():
        ...
        if spw_high_ghz < line_low_ghz:
            logger.warning('condition 1 not met') # added by me
            continue

        if spw_low_ghz > line_high_ghz:
            logger.warning('condition 2 not met') # added by me
            continue
        ...
        if max_chanwidth_ghz is not None:
            ...
            if spw_chanwidth_ghz > max_chanwidth_ghz:
                logger.warning('condition 3 not met') # added by me
                logger.info(f'spw_chanwidth_ghz= {spw_chanwidth_ghz}')
                continue

        if require_data:
            if len(vm.scansForSpw[this_spw]) == 0:
                logger.warning('condition 4 not met') # added by me
                continue

        if require_full_line_coverage and not (
                spw_high_ghz > line_high_ghz and spw_low_ghz < line_low_ghz):
            logger.warning('condition 5 not met') # added by me
            continue
